...
RSpec.describe TimedServiceRunner do
  describe ".run" do
    ...
    let(:service) { class_double(WasteCarriersEngine::UpdateAddressDetailsFromOsApiService) }
    ...
    before do
      ...
      allow(service).to receive(:name).and_return("UpdateAddressDetailsFromOsApiService")
    ...
    context "when an error occurs during service execution" do
      ...
      it "logs the error and notifies Airbrake" do
        ...
        expect(Rails.logger).to have_received(:error).at_least(:once).with("UpdateAddressDetailsFromOsApiService failed:\n Test error")
      end
    end
  end
end
