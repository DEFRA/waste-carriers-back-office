...
RSpec.describe WasteCarriersEngine::ApplicationHelper, type: :helper do
  let(:registration) { build(:renewing_registration) }
  let(:registration) { WasteCarriersEngine::Registration.where(reg_identifier: registration.reg_identifier).first }
  ...
  describe "#display_current_workflow_state" do
    ...
    context "when there is no matching translation" do
      before do
        registration.workflow_state = "foo"
      end
  ...
  describe "#display_expiry_date" do
    it "displays the correct date" do
      correct_expiry_date = Time.new(2000, 1, 2, 3, 4, 5).in_time_zone("UTC")
  ...
  describe "#display_registered_address" do
    context "when there is a registered address" do
      before do
        registration.addresses = [build(:address, :registered)]
      end
  ...
  describe "#display_contact_address" do
    context "when there is a contact address" do
      before do
        registration.addresses = [build(:address, :contact)]
      end
  ...
  describe "#key_people_with_conviction_search_results?" do
    ...
    context "when there are key people" do
      ...
      before do
        registration.key_people = [person]
      end
