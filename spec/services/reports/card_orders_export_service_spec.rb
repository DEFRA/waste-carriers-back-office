...
module Reports
  RSpec.describe CardOrdersExportService do
    describe ".run" do
      let(:file_name) { "waste_carriers_card_orders_weekly_#{Date.today.strftime('%Y-%m-%d')}" }
      let(:aws_file_pattern) { %r{https://.*\.s3\.eu-west-1\.amazonaws\.com/CARD_ORDERS/#{file_name}\.csv.*} }
