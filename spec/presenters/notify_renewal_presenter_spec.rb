...
RSpec.describe NotifyRenewalPresenter, type: :helper do
  ...
  describe "#registration_cost" do
    before { allow(Rails.configuration).to receive(:new_registration_charge).and_return(10_000) }

    it "returns the formatted new registration charge" do
      expect(presenter.registration_cost).to eq("100")
    end
  end

  describe "#renewal_cost" do
    before { allow(Rails.configuration).to receive(:renewal_charge).and_return(5_000) }

    it "returns the formatted renewal charge" do
      expect(presenter.renewal_cost).to eq("50")
    end
  end

  describe "#renewal_url" do
    before { allow(Rails.configuration).to receive(:wcrs_fo_link_domain).and_return("https://www.example.com") }
